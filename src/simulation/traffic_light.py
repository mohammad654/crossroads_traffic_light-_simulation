...
class TrafficLight:
    ...
    def draw(self, surface, camera_offset=(0, 0), scale=1.0):
        ...
        pulse = 0.85 + 0.15 * (1 + pygame.math.Vector2(1, 0).rotate(time_phase * 180).x) / 2
        glow_radius = int(max(3, light_radius * 2.2 * pulse))
        glow_surface = pygame.Surface((glow_radius * 2, glow_radius * 2), pygame.SRCALPHA)
        pygame.draw.circle(glow_surface, (*self.get_color(), 75), (glow_radius, glow_radius), glow_radius)
        surface.blit(glow_surface, (screen_x - glow_radius, screen_y + light_y_offset - glow_radius))

        if self.state == LightState.YELLOW:
            ring_radius = int(max(4, light_radius * (2.0 + 0.4 * pulse)))
            pygame.draw.circle(surface, (255, 210, 90), (int(screen_x), int(screen_y)), ring_radius, 1)
