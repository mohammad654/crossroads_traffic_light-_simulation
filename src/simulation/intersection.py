...
class Intersection:
    ...
    def setup_entry_exit_points(self):
        ...
        self.entry_lanes = {
            "north": [(center_x + offset, max_y - 8) for offset in inbound_offsets["north"]],
            "south": [(center_x + offset, min_y + 8) for offset in inbound_offsets["south"]],
            "east": [(min_x + 8, center_y + offset) for offset in inbound_offsets["east"]],
            "west": [(max_x - 8, center_y + offset) for offset in inbound_offsets["west"]]
        }

        self.exit_lanes = {
            "north": [(center_x + offset, min_y - 5) for offset in outbound_offsets["north"]],
            "south": [(center_x + offset, max_y + 5) for offset in outbound_offsets["south"]],
            "east": [(max_x + 5, center_y + offset) for offset in outbound_offsets["east"]],
            "west": [(min_x - 5, center_y + offset) for offset in outbound_offsets["west"]]
        }

        self.turning_lane_index = {
            "north": 0,
            "south": 0,
            "east": 0,
            "west": 0
        ...
        self.entry_points = {direction: lanes[0] for direction, lanes in self.entry_lanes.items()}
        self.exit_points = {direction: lanes[0] for direction, lanes in self.exit_lanes.items()}
    ...
    def get_spawn_position(self, direction, lane_index=None, turning=False):
        ...
        if direction in self.entry_lanes:
            ...
            if lane_index is None:
                lane_index = self.turning_lane_index[direction] if turning else random.randint(0, len(lanes) - 1)
    ...
    def get_turn_path(self, entry_direction, exit_direction):
        ...
        entry = self.get_spawn_position(entry_direction, lane_index=entry_lane, turning=True)
    ...
    def draw(self, surface, camera_offset=(0, 0), scale=1.0):
        ...
        for tx, ty in [(-56, -56), (56, -56), (-56, 56), (56, 56), (-80, 0), (80, 0), (0, -80), (0, 80)]:
        ...
        pygame.draw.rect(surface, shoulder_color, horizontal_road.inflate(0, 6), border_radius=8)
        ...
        pygame.draw.rect(surface, shoulder_color, vertical_road.inflate(6, 0), border_radius=8)
        ...
        pygame.draw.line(surface, solid_color, (camera_offset[0] + self.bounds[0] * scale, center_y), (camera_offset[0] + self.bounds[2] * scale, center_y), 2)
        pygame.draw.line(surface, solid_color, (center_x, camera_offset[1] + self.bounds[1] * scale), (center_x, camera_offset[1] + self.bounds[3] * scale), 2)
        ...
        for lane_y in divider_offsets:
            y = center_y + lane_y
            for x in range(int(camera_offset[0] + self.bounds[0] * scale), int(camera_offset[0] + self.bounds[2] * scale), int(dash_length + gap_length)):
                pygame.draw.line(surface, dashed_color, (x, y), (x + dash_length, y), 2)

        for lane_x in divider_offsets:
            x = center_x + lane_x
            for y in range(int(camera_offset[1] + self.bounds[1] * scale), int(camera_offset[1] + self.bounds[3] * scale), int(dash_length + gap_length)):
        ...
        for direction, lanes in self.entry_lanes.items():
            ...
            if direction in ["north", "south"]:
                pygame.draw.polygon(surface, turn_mark_color, [(tx - 4, ty), (tx + 4, ty), (tx, ty - 8 if direction == "north" else ty + 8)])
            else:
                pygame.draw.polygon(surface, turn_mark_color, [(tx, ty - 4), (tx, ty + 4), (tx + 8 if direction == "east" else tx - 8, ty)])
        ...
        for i in range(stripe_count):
            ...
            # North/South
            pygame.draw.line(surface, zebra_color,
                             (center_x + offset, center_y - height / 2 - 2 * scale),
                             (center_x + offset, center_y - height / 2 - zebra_stripe_len), zebra_width)
            pygame.draw.line(surface, zebra_color,
                             (center_x + offset, center_y + height / 2 + 2 * scale),
                             (center_x + offset, center_y + height / 2 + zebra_stripe_len), zebra_width)
            # East/West
            pygame.draw.line(surface, zebra_color,
                             ...
                             (center_x + width / 2 + zebra_stripe_len, center_y + offset), zebra_width)
            pygame.draw.line(surface, zebra_color,
                             (center_x - width / 2 - 2 * scale, center_y + offset),
                             (center_x - width / 2 - zebra_stripe_len, center_y + offset), zebra_width)
        ...
        pygame.draw.line(surface, stop_line_color,
                         ...
                         (center_x + road_width / 2, north_stop_y), stop_line_width)
        pygame.draw.line(surface, stop_line_color,
                         (center_x - road_width / 2, south_stop_y),
                         (center_x + road_width / 2, south_stop_y), stop_line_width)
        pygame.draw.line(surface, stop_line_color,
                         (east_stop_x, center_y - road_width / 2),
                         (east_stop_x, center_y + road_width / 2), stop_line_width)
        pygame.draw.line(surface, stop_line_color,
                         (west_stop_x, center_y - road_width / 2),
                         (west_stop_x, center_y + road_width / 2), stop_line_width)