...
class Renderer:
    def __init__(self, screen, simulation):
        ...
        self.sidebar_rect = pygame.Rect(10, 56, self.sidebar_width_expanded, self.height - 100)
        ...
        self.sliders = [
            Slider(0.2, 4.0, self.simulation.simulation_speed_factor, "Simulation Speed", step=0.01),
            Slider(0.2, 2.5, self.simulation.traffic_density_factor, "Traffic Density", step=0.05),
            Slider(10.0, 60.0, self.simulation.traffic_controller.default_phase_durations["ns_green"], "NS Green (s)", step=1.0),
            Slider(10.0, 60.0, self.simulation.traffic_controller.default_phase_durations["ew_green"], "EW Green (s)", step=1.0),
        ]
        ...
        self.manual_override_direction = None  # None | "north" | "south" | "east" | "west"
    ...
    def setup_ui_metadata(self):
        ...
        for algorithm in self.algorithms:
            self.tooltip_map[f"alg_{algorithm}"] = self.algorithm_descriptions.get(algorithm, "Traffic control algorithm")

    def add_notification(self, message, kind="info", ttl=2.0):
        self.notifications.append({"message": message, "kind": kind, "until": self.now() + ttl})
    ...
    def set_algorithm(self, algorithm):
        if algorithm != self.simulation.traffic_controller.algorithm:
            ...
            self.add_notification(f"Algorithm set: {self.format_algorithm_name(algorithm)}", "ok")
    ...
    def toggle_recording(self):
        if self.simulation.recording:
            ...
            self.add_notification(f"Recording stopped ({len(self.simulation.playback_frames)} frames)", "ok")
        else:
    ...
    def toggle_obstacle_tool(self):
        ...
        if self.adding_obstacle:
            self.adding_road_closure = False
        self.obstacle_drag_start = None
        self.add_notification("Obstacle tool " + ("enabled" if self.adding_obstacle else "disabled"), "info")
    ...
    def toggle_road_closure_tool(self):
        ...
        if self.adding_road_closure:
            self.adding_obstacle = False
        self.obstacle_drag_start = None
        self.add_notification("Road closure tool " + ("enabled" if self.adding_road_closure else "disabled"), "info")
    ...
    def is_over_ui(self, pos):
        if self.sidebar_rect.collidepoint(pos):
            return True
        if self.analytics_panel["visible"] and self.analytics_panel["rect"].collidepoint(pos):
            return True
    ...
    def handle_hotkeys(self, event):
        if event.key in (pygame.K_EQUALS, pygame.K_PLUS, pygame.K_KP_PLUS):
            ...
            self.add_notification("Heat map " + ("shown" if self.show_heatmap else "hidden"), "info")
        elif event.key == pygame.K_t:
            self.show_stats_card = not self.show_stats_card
            self.add_notification("Stats card " + ("shown" if self.show_stats_card else "hidden"), "info")
    ...
    def sync_slider_values(self):
        ...
        self.sliders[2].value = self.simulation.traffic_controller.default_phase_durations["ns_green"]
        self.sliders[2].input_text = f"{self.sliders[2].value:.2f}"
        self.sliders[3].value = self.simulation.traffic_controller.default_phase_durations["ew_green"]
    ...
    def apply_slider_value(self, slider):
        if slider.label == "Simulation Speed":
            ...
            self.add_notification(f"Simulation Speed = {slider.value:.2f}", "ok", ttl=1.2)
            ...
            self.add_notification(f"Traffic Density = {slider.value:.2f}", "ok", ttl=1.2)
        elif slider.label == "NS Green (s)":
            self.simulation.traffic_controller.set_phase_durations(ns_green=slider.value)
            ...
            self.simulation.traffic_controller.set_phase_durations(ew_green=slider.value)
    ...
    def handle_sidebar_click(self, pos):
        if self.sidebar_toggle_rect.collidepoint(pos):
            ...
            self.add_notification("Sidebar " + ("expanded" if self.sidebar_expanded else "collapsed"), "info")
            return True
        ...
        for slider in self.sliders:
            knob_rect = pygame.Rect(slider.knob_x() - 8, slider.track_rect.y - 7, 16, 22)
        ...
        for preset_key, rect in self.scenario_preset_rects.items():
            if rect.collidepoint(pos):
                ...
                self.sync_slider_values()
                self.add_notification(f"Scenario: {preset_key.replace('_', ' ').title()}", "ok")
                return True
            ...
            if rect.collidepoint(pos):
                if direction == "clear":
                    ...
                    if self.manual_override_direction == direction:
                        ...
                        self.simulation.traffic_controller.set_manual_override(direction, LightState.GREEN)
                        self.add_notification(f"Override: {direction.title()} GREEN", "warn")
                return True
    ...
    def _toggle_flag(self, attr_name, label):
        ...
        self.add_notification(label + " " + ("shown" if not current else "hidden"), "info")
    ...
    def handle_analytics_panel_interaction(self, event):
        ...
        if (close_rect and event.type == pygame.MOUSEBUTTONDOWN
                and event.button == 1 and close_rect.collidepoint(event.pos)):
            self.toggle_analytics_visibility()
            return True
    ...
    def update_hover_tooltip(self, pos):
        ...
        for algorithm, rect in self.algorithm_toggle_rects.items():
            if rect.collidepoint(pos):
                ...
                return
        for slider in self.sliders:
            if slider.track_rect.collidepoint(pos) or slider.input_rect.collidepoint(pos):
    ...
    def _draw_toolbar_button(self, rect, label, active=False, danger=False, warn=False, icon_color=None):
        ...
        pygame.draw.rect(shadow, (0, 0, 0, 28), pygame.Rect(2, 3, rect.width, rect.height), border_radius=9)
    ...
    def draw_section_header(self, x, y, w, label, accent_line=True):
        ...
        if accent_line:
            pygame.draw.rect(self.screen, self.theme["accent"], pygame.Rect(x, y + 2, 3, 12), border_radius=2)
        self.screen.blit(self.small_font.render(label.upper(), True, self.theme["muted"]), (x + 8, y))
        pygame.draw.line(self.screen, self.theme["border"], (x + 8, y + 18), (x + w, y + 18), 1)
        return y + 24

    def draw_panel(self, rect, radius=12, dark=False):
        # Layered shadow for depth
        shadow = pygame.Surface((rect.width + 12, rect.height + 12), pygame.SRCALPHA)
        pygame.draw.rect(shadow, (0, 0, 0, 35), pygame.Rect(6, 7, rect.width, rect.height), border_radius=radius + 2)
        pygame.draw.rect(shadow, (0, 0, 0, 20), pygame.Rect(3, 4, rect.width + 1, rect.height + 1), border_radius=radius + 1)
    ...
    def toggle_fullscreen(self):
        ...
        self.add_notification("Fullscreen " + ("on" if self.fullscreen else "off"), "info")
    ...
    def toggle_fast_forward(self):
        ...
        self.sync_slider_values()
        self.add_notification("Fast-forward " + ("ON (4×)" if self.fast_forward else "OFF"), "warn")

    def draw_toolbar(self):
        # ── App title pill ─────────────────────────────────────────────────
        toolbar_rect = pygame.Rect(10, 10, 310, 38)
        pygame.draw.rect(self.screen, self.theme["panel_dark"], toolbar_rect, border_radius=10)
        pygame.draw.rect(self.screen, self.theme["border_dark"], toolbar_rect, 1, border_radius=10)
        # vertical accent stripe on left edge
        pygame.draw.rect(self.screen, self.theme["accent"],
                         pygame.Rect(toolbar_rect.x, toolbar_rect.y + 8, 3, toolbar_rect.height - 16),
                         border_radius=2)
        title = self.title_font.render("Crossroads Traffic Simulation", True, self.theme["text_inv"])
        ...
        def _add_btn(button_id, label, action, w=80, active=False, danger=False, warn=False):
            nonlocal btn_x
            rect = pygame.Rect(btn_x, btn_y, w, btn_h)
            self._draw_toolbar_button(rect, label, active=active, danger=danger, warn=warn)
            self.toolbar_buttons.append({"id": button_id, "rect": rect, "action": action})
        ...
        def _divider():
            ...
            pygame.draw.line(self.screen, self.theme["border"],
                             (btn_x + 3, btn_y + 6), (btn_x + 3, btn_y + btn_h - 6), 1)
            btn_x += 10

        # ── GROUP 1: Playback control ───────────────────────────────────────
        is_running = not self.simulation.paused
        _add_btn("btn_play_pause",
                 "⏸ Pause" if is_running else "▶ Run",
                 self.toggle_pause, w=82, active=is_running)
        _add_btn("btn_reset", "↺ Reset", self.reset_simulation, w=72)
        _add_btn("btn_fast_fwd", "⏩ 4×" if not self.fast_forward else "⏩ 1×",
                 self.toggle_fast_forward, w=60, warn=self.fast_forward)
        _divider()

        # ── GROUP 2: Recording ─────────────────────────────────────────────
        _add_btn("btn_record",
                 "⏹ Stop" if self.simulation.recording else "⏺ Rec",
                 self.toggle_recording, w=72,
                 danger=self.simulation.recording)
        _add_btn("btn_playback", "▷ Play", self.start_playback, w=64)
        _divider()

        # ── GROUP 3: Tools ─────────────────────────────────────────────────
        _add_btn("btn_obstacle", "✚ Obstacle", self.toggle_obstacle_tool, w=82,
                 active=self.adding_obstacle)
        _add_btn("btn_closure", "⊘ Close", self.toggle_road_closure_tool, w=72,
                 active=self.adding_road_closure)
        _divider()

        # ── GROUP 4: Data / view ───────────────────────────────────────────
        _add_btn("analytics", "📊 Charts", self.toggle_analytics_visibility, w=78,
        ...
        _add_btn("btn_fullscreen", "⛶" if not self.fullscreen else "⛶ Exit",
                 self.toggle_fullscreen, w=52, active=self.fullscreen)
        _divider()

        # ── Safety / weather badge (right-anchored) ─────────────────────────
        safety_count = getattr(self.simulation.traffic_controller, "safety_violation_count", 0)
        weather_icons = {"clear": "☀", "rain": "☂", "fog": "≈", "snow": "❄"}
        w_icon = weather_icons.get(self.simulation.weather_mode, "")
        badge_label = (f"⚠ {safety_count} viol{'s' if safety_count != 1 else ''}"
                       if safety_count > 0 else f"✓ Safe  {w_icon} {self.simulation.weather_mode.title()}")
        ...
        self.tooltip_map["weather_badge"] = "W key cycles weather. Current: " + self.simulation.weather_mode
    ...
    def export_results(self):
        os.makedirs("data", exist_ok=True)
        export_file = os.path.join("data", f"simulation_results_{int(time.time())}.json")
    ...
    def toggle_analytics_visibility(self):
        ...
        self.add_notification("Analytics panel " + ("shown" if self.analytics_panel["visible"] else "hidden"), "info")
    ...
    def update_sidebar_animation(self):
        ...
        width = int(self.sidebar_width_collapsed + (self.sidebar_width_expanded - self.sidebar_width_collapsed) * self.sidebar_anim)
    ...
    def draw_tabs(self, x, y):
        ...
        for idx, tab in enumerate(self.tabs):
            ...
            self.screen.blit(self.small_font.render(tab, True, self.theme["text"]), (rect.x + 8, rect.y + 6))
    ...
    def draw_algorithm_toggles(self, x, y, available_w):
        ...
        for i, algorithm in enumerate(self.algorithms):
            ...
            self.screen.blit(self.small_font.render(label, True, text_color), (rect.x + 10, rect.y + 6))
            ...
            pygame.draw.circle(self.screen, self.theme["accent"] if active else (134, 146, 164), (knob_x, switch_rect.centery), 6)
            if active:
                pulse = int(70 + 40 * math.sin((self.now() - self.last_algorithm_change_time) * 10.0))
                glow = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
                pygame.draw.rect(glow, (59, 130, 246, max(15, min(80, pulse))), glow.get_rect(), border_radius=8)
    ...
    def draw_slider(self, slider, x, y, w):
        ...
        pygame.draw.rect(self.screen, (212, 222, 236), slider.track_rect, border_radius=6)
        fill_width = int((slider.value - slider.min_value) / max(1e-6, slider.max_value - slider.min_value) * slider.track_rect.width)
        fill_rect = pygame.Rect(slider.track_rect.x, slider.track_rect.y, max(2, fill_width), slider.track_rect.height)
        pygame.draw.rect(self.screen, self.theme["accent"], fill_rect, border_radius=6)
        pygame.draw.circle(self.screen, (255, 255, 255), (slider.knob_x(), slider.track_rect.centery), 8)
        pygame.draw.circle(self.screen, self.theme["accent"], (slider.knob_x(), slider.track_rect.centery), 8, 2)

        changed_recently = self.now() - self.last_param_change.get(slider.label, -999) < 0.75
        input_color = self.theme["accent_soft"] if (slider.editing or changed_recently) else self.theme["panel_2"]
        pygame.draw.rect(self.screen, input_color, slider.input_rect, border_radius=7)
        pygame.draw.rect(self.screen, self.theme["border"], slider.input_rect, 1, border_radius=7)
        text = slider.input_text if slider.editing else f"{slider.value:.2f}"
        self.screen.blit(self.small_font.render(text, True, self.theme["text"]), (slider.input_rect.x + 8, slider.input_rect.y + 6))
    ...
    def draw_sidebar(self):
        ...
        # ── Collapse toggle ──────────────────────────────────────────────────
        self.sidebar_toggle_rect = pygame.Rect(self.sidebar_rect.right - 34, self.sidebar_rect.y + 10, 24, 24)
        pygame.draw.rect(self.screen, self.theme["panel_dark"], self.sidebar_toggle_rect, border_radius=7)
        pygame.draw.rect(self.screen, self.theme["border_dark"], self.sidebar_toggle_rect, 1, border_radius=7)
        arrow = "◀" if self.sidebar_expanded else "▶"
        arrow_text = self.small_font.render(arrow, True, self.theme["text_inv"])
        self.screen.blit(arrow_text, arrow_text.get_rect(center=self.sidebar_toggle_rect.center))
        ...
        pygame.draw.rect(self.screen, self.theme["panel_dark"], hdr_rect,
        ...
        phase_colors = {
            "ns_green": self.theme["ok"], "ew_green": self.theme["ok"],
            "ns_yellow": self.theme["warn"], "ew_yellow": self.theme["warn"],
        }
        ...
        pygame.draw.rect(self.screen, self.theme["panel_2"], phase_rect, border_radius=8)
        # Left accent bar
        pygame.draw.rect(self.screen, phase_bg,
                         pygame.Rect(phase_rect.x, phase_rect.y + 4, 4, phase_rect.height - 8), border_radius=2)
        pygame.draw.rect(self.screen, phase_bg, phase_rect, 1, border_radius=8)
        self.screen.blit(self.small_font.render(f"Mode: {mode_name}", True, self.theme["muted"]),
                         (phase_rect.x + 12, phase_rect.y + 6))
        self.screen.blit(self.small_font.render(f"{phase_label}  —  {phase_remaining:.1f}s", True, phase_bg),
        ...
        pbar = pygame.Rect(phase_rect.x + 4, phase_rect.bottom - 4, phase_rect.width - 8, 3)
        pygame.draw.rect(self.screen, self.theme["border"], pbar, border_radius=2)
        pygame.draw.rect(self.screen, phase_bg,
                         pygame.Rect(pbar.x, pbar.y, int(pbar.width * pbar_frac), pbar.height), border_radius=2)
        content_y += 56

        # ── Section: Simulation Parameters ─────────────────────────────────
        content_y = self.draw_section_header(x, content_y, content_w, "Simulation Parameters")
        slider_y = content_y
        for slider in self.sliders:
        ...
        # ── Section: Algorithm ──────────────────────────────────────────────
        content_y = self.draw_section_header(x, content_y, content_w, "Traffic Algorithm")
        ...
        # ── Section: Scenarios ───────────────────────────────────────────────
        content_y = self.draw_section_header(x, content_y, content_w, "Scenario Presets")
        self.scenario_preset_rects = {}
        presets = [("normal_day", "🌤 Normal"), ("rush_hour", "🚗 Rush"), ("special_event", "🎪 Event")]
        btn_w = (content_w - 8) // 3
        for idx, (key, label) in enumerate(presets):
            rect = pygame.Rect(x + idx * (btn_w + 4), content_y, btn_w, 26)
            pygame.draw.rect(self.screen, self.theme["panel_2"], rect, border_radius=8)
            pygame.draw.rect(self.screen, self.theme["border"], rect, 1, border_radius=8)
            self.screen.blit(self.small_font.render(label, True, self.theme["text"]),
                             self.small_font.render(label, True, self.theme["text"]).get_rect(center=rect.center).topleft)
        ...
        self.manual_override_rects = {}
        override_dirs = [("north", "↑ N"), ("east", "→ E"), ("south", "↓ S"), ("west", "← W")]
        ...
        pygame.draw.rect(self.screen, self.theme["panel_2"], clear_rect, border_radius=7)
        pygame.draw.rect(self.screen, self.theme["border"], clear_rect, 1, border_radius=7)
        clear_txt = self.small_font.render("✕ Clear Override", True, self.theme["muted"])
        self.screen.blit(clear_txt, clear_txt.get_rect(center=clear_rect.center))
        self.manual_override_rects["clear"] = clear_rect
        content_y += 28

        # ── Keyboard shortcuts footer ─────────────────────────────────────────
        footer_rect = pygame.Rect(x - 2, self.sidebar_rect.bottom - 90, content_w + 4, 78)
        pygame.draw.rect(self.screen, self.theme["panel_dark"], footer_rect,
                         border_bottom_left_radius=12, border_bottom_right_radius=12)
        pygame.draw.rect(self.screen, self.theme["border_dark"], footer_rect, 1,
                         border_bottom_left_radius=12, border_bottom_right_radius=12)
        klines = [
        ...
        for i, line in enumerate(klines):
            self.screen.blit(self.small_font.render(line, True, self.theme["border_dark"]),
    ...
    def draw_interactive_line_chart(self, surface, rect, values, color, label, chart_id):
        ...
        if len(series) < 2:
            surface.blit(self.small_font.render(label, True, self.theme["muted"]), (rect.x + 8, rect.y + 8))
            return
        ...
        surface.blit(self.small_font.render(f"{label}  z:{view['zoom']:.1f}", True, self.theme["muted"]), (rect.x + 7, rect.y + 5))

    def draw_comparison_chart(self, surface, rect):
        pygame.draw.rect(surface, (252, 253, 255), rect, border_radius=8)
        pygame.draw.rect(surface, self.theme["border"], rect, 1, border_radius=8)
        if not self.comparison_mode:
            surface.blit(self.small_font.render("Comparison mode disabled", True, self.theme["muted"]), (rect.x + 8, rect.y + 8))
            return
        ...
        for _, color, label in datasets:
            ...
            surface.blit(self.small_font.render(label, True, self.theme["muted"]), (rect.right - 84, legend_y))
            legend_y += 16
    ...
    def render_analytics_panel(self):
        ...
        pygame.draw.rect(self.screen, self.theme["panel_dark"], header_rect,
                         border_top_left_radius=12, border_top_right_radius=12)
        pygame.draw.rect(self.screen, self.theme["accent"],
                         pygame.Rect(header_rect.x, header_rect.bottom - 2, header_rect.width, 2))
        # Chart icon + title
        self.screen.blit(self.title_font.render("📊 Realtime Analytics", True, self.theme["text_inv"]),
        ...
        content_rect = pygame.Rect(rect.x + 10, rect.y + 46, rect.width - 20, rect.height - 58)
        self.analytics_content_rect = content_rect

        refresh_stamp = len(self.simulation.wait_time_history) + len(self.simulation.throughput_history) + len(self.simulation.avg_speed_history)
        need_refresh = (
            self.analytics_cache_surface is None
            or self.analytics_cache_surface.get_size() != (content_rect.width, content_rect.height)
            or refresh_stamp != self.analytics_cache_stamp
            or self.analytics_panels_dirty
        )

        if need_refresh:
            self.analytics_cache_surface = pygame.Surface((content_rect.width, content_rect.height), pygame.SRCALPHA)
            ...
            throughput_rect = pygame.Rect(chart_w + 10, 0, content_rect.width - chart_w - 10, chart_h)
            speed_rect = pygame.Rect(0, chart_h + 10, chart_w, content_rect.height - chart_h - 10)
            compare_rect = pygame.Rect(chart_w + 10, chart_h + 10, content_rect.width - chart_w - 10, content_rect.height - chart_h - 10)

            self.draw_interactive_line_chart(self.analytics_cache_surface, wait_rect, self.simulation.wait_time_history, (224, 93, 86), "Wait", "wait")
            self.draw_interactive_line_chart(self.analytics_cache_surface, throughput_rect, self.simulation.throughput_history, (64, 156, 90), "Throughput", "throughput")
            self.draw_interactive_line_chart(self.analytics_cache_surface, speed_rect, self.simulation.avg_speed_history, (82, 116, 231), "Speed", "speed")
        ...
        table_rect = pygame.Rect(rect.x + 12, rect.bottom - table_h - 8, rect.width - 24, table_h)
        pygame.draw.rect(self.screen, (252, 253, 255), table_rect, border_radius=8)
        pygame.draw.rect(self.screen, self.theme["border"], table_rect, 1, border_radius=8)
        if comparison:
            header = self.small_font.render("Algorithm        Queue  Delay  Thru", True, self.theme["muted"])
            ...
            for row_idx, (name, metrics) in enumerate(list(comparison.items())[:5]):
                ...
                self.screen.blit(self.small_font.render(row_txt, True, color), (table_rect.x + 8, row_y))
        else:
            self.screen.blit(self.small_font.render("Algorithm metrics collecting...", True, self.theme["muted"]), (table_rect.x + 8, table_rect.y + 13))

        resize_rect = pygame.Rect(rect.right - 16, rect.bottom - 16, 16, 16)
        pygame.draw.polygon(
            self.screen,
            self.theme["muted"],
            [(resize_rect.left + 4, resize_rect.bottom - 4), (resize_rect.right - 4, resize_rect.bottom - 4), (resize_rect.right - 4, resize_rect.top + 4)],
            0,
        )
    ...
    def render_status_bar(self):
        ...
        pygame.draw.rect(self.screen, self.theme["border_dark"], rect, 1, border_radius=8)

        alg_label = self.format_algorithm_name(self.simulation.traffic_controller.algorithm)
        mode_tags = []
        ...
        self.screen.blit(self.small_font.render(state, True, self.theme["text_inv"]),
                         (rect.x + 8, rect.y + 6))
        ...
        if self.simulation.recording and self.simulation.recorded_frames:
            ...
            bar_w = 120
            bar_rect = pygame.Rect(rect.right - bar_w - 10, rect.y + 6, bar_w, 12)
            pygame.draw.rect(self.screen, self.theme["border"], bar_rect, border_radius=4)
            fill = pygame.Rect(bar_rect.x, bar_rect.y, int(bar_w * progress), 12)
    ...
    def render_notifications(self):
        ...
        for idx, note in enumerate(self.notifications[-4:]):
            ...
            if note["kind"] == "ok":
                ...
                bg, border, fg = self.theme["panel_dark"], self.theme["border_dark"], self.theme["text_inv"]
            notif_surf = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
            pygame.draw.rect(notif_surf, (*bg, alpha), notif_surf.get_rect(), border_radius=8)
            pygame.draw.rect(notif_surf, (*border, alpha), notif_surf.get_rect(), 1, border_radius=8)
    ...
    def render_tooltip(self):
        ...
        rect = pygame.Rect(pos[0] + 14, pos[1] + 14, text.get_width() + 12, text.get_height() + 8)
    ...
    def render(self):
        ...
        for ty in range(0, self.height, tile):
            for tx in range(0, self.width, tile):
                c = self.theme["bg_a"] if ((tx // tile + ty // tile) % 2 == 0) else self.theme["bg_b"]
    ...
    def render_stats_card(self):
        ...
        pygame.draw.rect(self.screen, self.theme["accent"], pygame.Rect(hdr.x, hdr.bottom - 14, hdr.width, 14))
        self.screen.blit(self.title_font.render("Simulation Stats", True, (255, 255, 255)), (hdr.x + 12, hdr.y + 8))

        dashboard = self.simulation.get_dashboard_metrics()
        active_lights = ", ".join(
            [d[0].upper() for d, light in self.simulation.traffic_lights.items() if light.state.name == "GREEN"]
        ) or "None"
        ...
        safety_txt   = "Safety: OK" if safety_count == 0 else f"Safety: {safety_count} violation(s)"
        ...
        lines = [
            (f"Vehicles: {dashboard['vehicles_total']}  Exited: {dashboard['exited_total']}",
             self.theme["text"]),
            (f"Elapsed: {self.simulation.elapsed_time:.0f}s",
             self.theme["muted"]),
            (f"Avg Wait: {dashboard['avg_wait']:.2f}s   Throughput: {dashboard['throughput']:.2f}/s",
             self.theme["text"]),
            (f"Queue  NS:{queue['ns']}  EW:{queue['ew']}",
             self.theme["text"]),
            (f"Avg Speed: {dashboard['avg_speed']:.1f} m/s",
             self.theme["muted"]),
            (f"Close Calls: {dashboard['close_calls']}   Pedestrians: {dashboard['pedestrians']}",
             self.theme["warn"] if dashboard["close_calls"] > 0 else self.theme["muted"]),
            (f"Green dirs: {active_lights}",
             self.theme["ok"]),
            (f"Phase: {phase_name}  {phase_rem:.1f}s left",
             self.theme["accent"]),
            (f"Weather: {weather_icon}{dashboard['weather'].title()}  Rush: {'ON' if dashboard['rush_hour'] else 'off'}",
             self.theme["muted"]),
            (f"Mix: {mix_str}" if mix_str else "Mix: —",
             self.theme["muted"]),
        ]
        ...
        # Safety status bar
        safety_bar = pygame.Rect(stats_rect.x + 8, stats_rect.bottom - 28, stats_rect.width - 16, 20)
        pygame.draw.rect(self.screen, safety_color, safety_bar, border_radius=7)
        self.screen.blit(self.small_font.render(safety_txt, True, (255, 255, 255)),
    ...
    def render_day_night_overlay(self):
        phase = (self.simulation.elapsed_time % self.day_night_cycle_seconds) / self.day_night_cycle_seconds
    ...
    def render_weather_effects(self):
        ...
        for particle in self.weather_particles:
            ...
            if particle["y"] > self.height + 4 or particle["x"] < -4:
                particle["x"] = float((pygame.time.get_ticks() * 0.13) % max(1, self.width))
                particle["y"] = -5.0

            if weather == "rain":
                pygame.draw.line(self.screen, (170, 205, 255, 140), (int(particle["x"]), int(particle["y"])), (int(particle["x"] + 2), int(particle["y"] + particle["len"])), 1)
            elif weather == "snow":
                pygame.draw.circle(self.screen, (245, 245, 255, 180), (int(particle["x"]), int(particle["y"])), 2)
            elif weather == "fog":
    ...
    def render_minimap(self):
        ...
        def to_map(point):
            ...
            py = map_rect.y + 24 + int(((point[1] - min_y) / h) * (map_rect.height - 34))
            return px, py
