...
class TrafficLightController:
    ...
    def __init__(self, safety_checker):
        ...
        self.phase_sequence = ["ns_green", "ns_yellow", "all_red_1", "ew_green", "ew_yellow", "all_red_2"]
        self.phase_index = 0
    ...
    def update_adaptive(self, dt, context=None):
        ...
        learning_rate = 0.035
        self.adaptive_memory["ns"] = (1 - learning_rate) * self.adaptive_memory["ns"] + learning_rate * ns_score
        self.adaptive_memory["ew"] = (1 - learning_rate) * self.adaptive_memory["ew"] + learning_rate * ew_score
        ...
        self.current_phase_durations = self.default_phase_durations.copy()
        self.current_phase_durations["ns_green"] = max(14.0, min(34.0, 16.0 + 18.0 * (ns_score / total)))
        self.current_phase_durations["ew_green"] = max(14.0, min(34.0, 16.0 + 18.0 * (ew_score / total)))
        self._advance_phase_machine(dt)
    ...
    def get_pedestrian_walk_states(self):
        ...
        ns_restrictive = north.state in [LightState.RED] and south.state in [LightState.RED]
        ew_restrictive = east.state in [LightState.RED] and west.state in [LightState.RED]
        return {
            "ns_cross": ns_restrictive,
            "ew_cross": ew_restrictive
        }
    ...
    def update_ml_optimized(self, dt, context=None):
        ...
        self.current_phase_durations["ew_green"] = max(16.0, 48.0 - self.ml_last_duration)
        self._advance_phase_machine(dt)
    ...
    def _update_algorithm_metrics(self, context):
        ...
        metrics["avg_delay"] = (1 - alpha) * metrics["avg_delay"] + alpha * avg_wait_time
    ...
    def set_algorithm(self, algorithm):
        ...
        if algorithm in self.available_algorithms:
            ...
            LOGGER.warning("Unknown algorithm requested", extra={"algorithm": algorithm})

    def set_phase_durations(self, ns_green=None, ew_green=None, ns_yellow=None, ew_yellow=None, all_red=None):
